...
/** Find Z.AI API credentials from environment. */
export async function findApiKey(): Promise<string | null> {
	return findCredential(getEnvApiKey("zai"));
}
...
export async function searchZai(params: ZaiSearchParams): Promise<SearchResponse> {
	...
	if (!apiKey) {
		throw new Error("Z.AI credentials not found. Set ZAI_API_KEY in environment.");
	}
