 ...
 *
 * Simplified: only supports ANTHROPIC_API_KEY (no OAuth).
 */
...
function getModel(): string {
	return process.env.ANTHROPIC_SEARCH_MODEL ?? DEFAULT_MODEL;
}
...
export async function searchAnthropic(params: AnthropicSearchParams): Promise<SearchResponse> {
	...
	if (!apiKey) {
		throw new Error("No Anthropic credentials found. Set ANTHROPIC_API_KEY in environment.");
	}
