...
/** Find Synthetic API key from environment. */
export async function findApiKey(): Promise<string | null> {
	return findCredential(getEnvApiKey("synthetic"));
}
...
export async function searchSynthetic(params: {
	...
	if (!apiKey) {
		throw new Error("Synthetic credentials not found. Set SYNTHETIC_API_KEY in environment.");
	}
