...
import { buildResult, MAX_BYTES } from "../fetch/types.js";
...
function getDocsRsCachePath(target: DocsRsTarget, now = new Date()): string {
	...
	const cacheDir = path.join(os.tmpdir(), "pi-web-ext", DOCS_RS_CACHE_ROOT);
	return path.join(cacheDir, `docsrs_${crate}_${version}`, DOCS_RS_CACHE_FILENAME);
}
