...
/** Find Tavily API key from environment. */
export async function findApiKey(): Promise<string | null> {
	return findCredential(getEnvApiKey("tavily"));
}
...
export async function searchTavily(params: TavilySearchParams): Promise<SearchResponse> {
	...
	if (!apiKey) {
		throw new Error("Tavily credentials not found. Set TAVILY_API_KEY in environment.");
	}
