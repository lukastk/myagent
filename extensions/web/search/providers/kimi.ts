...
import { findCredential, isApiKeyAvailable } from "./utils.js";
...
function resolveBaseUrl(): string {
	return asTrimmed(process.env.MOONSHOT_SEARCH_BASE_URL) ?? asTrimmed(process.env.KIMI_SEARCH_BASE_URL) ?? KIMI_SEARCH_URL;
}

/** Find Kimi search credentials from environment. */
async function findApiKey(): Promise<string | null> {
	const envKey =
		asTrimmed(process.env.MOONSHOT_SEARCH_API_KEY) ??
		asTrimmed(process.env.KIMI_SEARCH_API_KEY) ??
		getEnvApiKey("kimi") ??
		null;
	return findCredential(envKey);
}
