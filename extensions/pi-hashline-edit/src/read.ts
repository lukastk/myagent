...
export function registerReadTool(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    renderResult(result, { expanded }, theme, context) {
      const imageContent = result.content.find((block) => block.type === "image");
      if (imageContent) {
        return expanded ? new Text("", 0, 0) : new Text(theme.fg("muted", " → image"), 0, 0);
      }
      ...
      if (context.isError) {
        const firstLine = text.split("\n").find((line) => line.trim().length > 0) ?? "read failed";
        return new Text(theme.fg("error", ` → ${firstLine}`), 0, 0);
      }

      const hashlineCount = text
        .split("\n")
        .filter((line) => /^\d+#\w+:/.test(line))
        .length;
      if (hashlineCount > 0) {
        return new Text(theme.fg("muted", ` → ${hashlineCount} lines`), 0, 0);
      }
